...
def cut_ctr(array: np.ndarray, sizes: tuple, center_crop: bool = False) -> np.ndarray:
    """
    Crop a multi-dimensional array either at random positions or at its center, according to sizes.
    :param array: multidimensional array
    :param sizes: tuple of the same length as array.ndim
    :param center_crop: bool, if True performs center crop else performs random crop
    :return: cropped multidimensional array
    """
    array = array.copy()
    ...
    for axis in range(array.ndim):
        ...
        if axis_target_size > axis_original_size:
            ...
            if center_crop:
                ...
                axis_start_idx = np.random.randint(0, axis_original_size - axis_target_size + 1)

            axis_end_idx = axis_start_idx + axis_target_size

            # Crop the array along the axis
            array = np.take(array, np.arange(axis_start_idx, axis_end_idx), axis)

    assert array.shape == sizes, 'The input size and the size of the cropped image must be the same'
...
def wiener_adaptive(x: np.ndarray, noise_var: float, **kwargs) -> np.ndarray:
    ...
    x = x * noise_var / (coef_var_min + noise_var + 1e-8)
