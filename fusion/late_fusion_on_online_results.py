...
def main():
    ...
    n_run_fusion_dir = os.path.join(args.project_dir, 'fusion', 'results', f'winSize{args.extend_duration_sec}sec',
                                    'ProductRuleLoss', f'visual{args.visual_model}_audio{args.audio_model}',
                                    args.data_content, exp_setup, f"fold{args.n_fold}",
                                    f'run{args.n_run_visual}')
    print(f'Run fusion dir: {n_run_fusion_dir}')
    ...
    with open(os.path.join(n_run_fusion_dir, 'stats', 'audio_stats.pkl'), "rb") as f:
    ...
    with open(os.path.join(n_run_fusion_dir, 'stats', 'visual_stats.pkl'), "rb") as f:
        visual_stats = pkl.load(f)

    fusion_checkpoint = torch.load(os.path.join(n_run_fusion_dir, "fusion_model_best.ckpt"), map_location='cpu')
    best_val_epoch = fusion_checkpoint['epoch']
    print(f'Best epoch {best_val_epoch}')

    per_image_frame_probs_dict = {
        dev_sample_id: {k: v["frame_probabilities"] for k, v in frame_values.items()}
        for dev_sample_id, frame_values in visual_stats.items()
    }
    ...
    if plt_cm:
        ...
        save_cm_stats(videos_audio_cm, classes=class_names, normalize=False,
                      title=f"model_best_epoch_{best_val_epoch}_{total_audio_vid_acc:.04f}_audio_vid",
        ...
        videos_audio_cm = confusion_matrix(y_true=per_frame_audio_gnd_labels, y_pred=per_frame_audio_pred_labels,
                                           labels=range(0, 34))
        save_cm_stats(videos_audio_cm, classes=class_names, normalize=False,
                      title=f"model_best_epoch_{best_val_epoch}_{total_audio_frame_acc:.04f}_audio_frame",
                      save_dir=None, figsize=(20, 20))

    # Compute Visual Accuracy results
    per_vid_visual_acc, per_frame_visual_acc = compute_video_frame_acc(visual_stats, device_sample_names)
    ...
    if plt_cm:
        ...
        save_cm_stats(videos_visual_cm, classes=class_names, normalize=False,
                      title=f"model_best_epoch_{best_val_epoch}_{total_visual_vid_acc:.04f}_visual_vid",
                      save_dir=None, figsize=(20, 20))
        ...
        save_cm_stats(videos_visual_cm, classes=class_names, normalize=False,
                      title=f"model_best_epoch_{best_val_epoch}_{total_visual_frame_acc:.04f}_visual_frame",
                      save_dir=None, figsize=(20, 20))
    ...
    for dev_sample_id in device_sample_names:
        ...
        assert all(np.array(list(per_audio_frame_probs_dict[dev_sample_id].keys()))
                   == np.array(list(per_image_frame_probs_dict[dev_sample_id].keys())))
        ...
        assert all(np.array(list(per_audio_frame_pred_dict[dev_sample_id].keys()))
                   == np.array(list(per_image_frame_pred_dict[dev_sample_id].keys())))
    ...
    if plt_cm:
        ...
        save_cm_stats(videos_prod_cm, classes=class_names, normalize=False,
                      title=f"model_best_epoch_{best_val_epoch}_{total_prod_vid_acc:.04f}_prod_vid",
                      save_dir=None, figsize=(20, 20))
        ...
        save_cm_stats(frames_prod_cm, classes=class_names, normalize=False,
                      title=f"model_best_epoch_{best_val_epoch}_{total_prod_frame_acc:.04f}_prod_frame",
                      save_dir=None, figsize=(20, 20))
